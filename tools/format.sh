...
while IFS= read -r f; do
  case "$f" in
    ...
    # would be undone by the next `tools/build-plugins.sh`. One glob rather than
    # a line per guest, so adding a C++ guest cannot forget this.
    examples/plugins/*/bindings/*) continue ;;
  esac
  files+=("$f")
done < <(
