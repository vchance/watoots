...
- **WASI:** 0.2.12 is the stable, universally-supported target. 0.3.0 was ratified 2026-06-11 (async, `stream<T>`/`future<T>`, `wasi:io` deleted). **Decided in [ADR-0013](adr/0013-wasi-p3-position.md): stay on 0.2, with two written conditions for adopting p3.** The disqualifying one is that `wasmtime-wasi`'s p3 module states that bug and security fixes limited to wasip3 get no patch release — verified verbatim in 48.0.1 and again in 48.0.2 — which a sandbox cannot depend on. The ecosystem has moved regardless, and the ADR says so: Spin 4.1.0 bumps to Wasmtime 48.0.0 and carries p3 through core features, wasmCloud has built with WASI 0.3 always on since 2.5.0 — though on Wasmtime 46/47 rather than the 48 LTS, and serving p2 and p3 worlds side by side — and wasi-sdk 34 ships a p3 sysroot. Two claims previously on this line were wrong and are corrected there: `wasm32-wasip3` is still **Tier 3** in the rustc book (the *promotion proposal* was accepted 2026-08-09, which is not the same thing, and `rustup` does not offer the target on our toolchain), and it is Spin 4.1.0 rather than 4.0. The permission model being "0.3-shaped" is now enforced by `crates/host/tests/wasip3_shape.rs` rather than asserted — writing it found a live bug, in which 0.3's renamed wall clock classified as monotonic.
