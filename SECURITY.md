...
**0.6.x** is the supported line. Fixes land on `main` and go out in the next
0.6.x tag; there is no backport branch, and older lines are not maintained. The only versions on crates.io are the `0.0.0` placeholders that
